export const textGoodsCard = {
  volume: "Объём",
  volumeUnit: "л",
  stock: "В наличии",
  stockUnit: "шт",
  btnText: "В корзину",
  currency: "₽",
  imgAlt: "Фото товара.",
  ratingAlt: "Звезда.",
  delAlt: "Кнопка удаления товара.",
};

export const textSignUp = {
  title: "Регистрация",
  name: "Фамилия Имя Отчество",
  email: "Почта",
  tel: "Номер телефона",
  password: "Пароль",
  agreement: "Согласен с",
  privacyPolicy: "политикой конфиденциальности",
  btn: "Регистрация",
};

export const textPromoCard = {
  imgAlt: "Картинка акции.",
};

export const textPartnershipCard = {
  imgAlt: "Картинка предложения.",
};

export const textFeedback = {
  ratingAlt: "Звезда",
  dignity: "Достоинства: ",
  defects: "Недостатки: ",
};

export const textOrder = {
  iconActiveAlt: "Кнопка удаления заказа.",
  iconDoneAlt: "Иконка выполненного заказа.",
  iconArrowAlt: "Кнопка открытия подробной информации.",
  from: "от",
  currency: "₽",
  amountUnit: "шт",
};

export const textFeedbackCard = {
  title: "Отзывы",
  allFeedback: "Смотреть все отзывы...",
  btnText: "Оставить отзыв",
};

export const textRecomendationCard = {
  title: "Также рекомендуем",
  btnText: "Перейти в каталог",
};

export const textGoodsDetailsCard = {
  stock: "В наличии",
  stockUnit: "шт",
  producer: "Производитель:",
  brand: "Бренд:",
  ibu: "IBU:",
  style: "Стиль:",
  color: "Цвет:",
  density: "Плотность:",
  strength: "Крепость:",
  taste: "Вкус:",
  volume: "Объём:",
  volumeUnit: "л",
  points: "Вы можете оплатить до 100% баллами",
  btnText: "В корзину",
  currency: "₽",
  imgAlt: "Фото товара.",
  ratingAlt: "Звезда.",
  description: "Описание",
};

export const textReservationCard = {
  title1: "Корзина",
  title2: "Заказ успешно сформирован",
  text: "Обратите внимание, что заказ нужно оплатить и забрать сегодня до 22:00 по адресу:",
  address: "г. Люберцы, ул. 3-е почтов. отделение, дом 49, корп. 2",
  orderNumber: "Номер заказа",
  cost: "К оплате",
  currency: "₽",
};

export const textOrderCostCard = {
  title: "Сумма заказа",
  cost: "Сумма:",
  points: "Ваши баллы:",
  currency: "₽",
  writeOff: "Списать баллы",
  label: "Введите колличество баллов для списания",
  btnText: "Сформировать заказ",
};

export const textFooter = {
  logoAlt: "Логотип.",
  socialsAlt: "Иконка соцсети.",
  main: "Главная",
  loyalty: "О программе лояльности",
  assortment: "Ассортимент",
  partners: "Партнёрам",
  copyright: "probkishop.ru",
};

export const textHeader = {
  tel: "+7 (968)-819-01-89",
  placeholder: "Поиск по сайту",
  locationAlt: "Иконка геолокации.",
  logoAlt: "Логотип",
  iconAlt: "Иконка меню.",
  zhukovskiy: "г. Жуковский",
  main: "Главная",
  catalog: "Каталог",
  loyalty: "Программа лояльности",
};

export const textMainPage = {
  title: "Самые лучшие напитки от зарубежных и отечественных пивоварен",
  btnText: "Перейти в каталог",
  missionTitle: "Наша миссия",
  missionText:
    "Мы прививаем культуру потребления пива, предлагая клиентам качественную продукцию, рассказывая об алкоголе с другой стороны, открывая новые горизонты. Мы даём радость, общение и повод для встреч!",
  clarification1: "500+",
  clarification1Text: "Видов крафтового пива",
  clarification2: "60+",
  clarification2Text: "Сортов пива на кранах",
  clarification3: "130+",
  clarification3Text: "Видов закусок и рыбы",
  slide1:
    "Вы можете арендовать у нас оборудование на время проведения мероприятия, нужно лишь перейти в каталог",
  novaltyTitle: "Новинки",
  novaltySubtitle: "Товары, которые поступили недавно",
  ourShopsTitle: "Наши магазины",
  ourShopsSubtitle: "Мы ближе, чем вы думаете",
  choiceShop: "Выберите магазин из выпадающего списка:",
  weekdays: "Понедельник-пятница",
  weekend: "Суббота, Воскресенье",
  promoTitle: "Акции",
  promoSubtitle: "Наши лучшие предложения",
  promo1Title: "2+1",
  promo1Text: "За каждую покупку 2 литров пива вы получите 1 в подарок",
  promo2Title: "Приведи друга",
  promo2Text:
    "И на карту лояльности будет начисляться 2% от суммы покупок друга",
  promo3Title: "День рождения",
  promo3Text: "Скидка 10% на всё в день рождения, неделю до и после него",
  collabTitle: "Давайте сотрудничать",
  collabSubtitle: "Развивайте бизнес вместе с нами",
  collab1Title: "Нашим поставщикам",
  collab1Text:
    "Хотите предложить свою продукцию для продажи в сети магазинов «PROБКИ»?",
  collab2Title: "Аренда помещений",
  collab2Text: "Мы всегда в поисках новых помещений для новых магазинов",
  collabBtnText: "Подробнее об условиях",
  collabAlt: "Фото магазина",
};

export const textStartPage = {
  logoAlt: "Логотип.",
  title: "Вы старше 18 лет?",
  agree: "Да",
  disagree: "Нет",
  alarm: "Извините, наш сайт предназначен только для совершеннолетних. =(",
};

export const textNotFoundPage = {
  iconAlt: "Страница не найдена.",
  text: "К сожалению данной страницы не существует, но вскоре мы это исправим!",
  btnText: "Перейти в каталог",
};

export const textLoyaltyPage = {
  title: "Как работает программа?",
  text1: "Зарегистрируйтесь на сайте.",
  text2: "3% от суммы чека будут начисляться на карту в виде баллов.",
  text3: "Накопленными баллами можно оплатить до 100% покупок.",
  text4:
    "Также, если вы приведёте к нам друга, вам будет начисляться ещё 2% от суммы покупок друга.",
  imgAlt: "Фото магазина.",
};

export const textPrtnershipPage = {
  imgAlt: "Фото магазина.",
  title1: "Нашим поставщикам",
  title2: "Аренда помещений",
  text1_1:
    "Хотите предложить свою продукцию для продажи в сети магазинов «PROБКИ»?",
  text1_2: "Напишите нам на info@probkishop.ru и расскажите о:",
  text1_3: "Группе товара",
  text1_4: "Название компании",
  text1_5: "Описание компании",
  text1_6: "Ассортимент",
  text1_7: "Прайс",
  text1_8: "Веб-сайт (если есть)",
  text1_9: "Мы обязательно с вами свяжемся!",
  text2_1: "Мы всегда в поисках новых помещений для новых магазинов.",
  text2_2: "Напишите нам на info@probkishop.ru и расскажите о:",
  text2_3: "Адрес",
  text2_4: "Площадь",
  text2_5: "Арендная ставка за м.кв",
  text2_6: "Этаж",
  text2_7: "Мы обязательно с вами свяжемся!",
};

export const textPAPage = {
  title: "Личный кабинет",
  greeting1: "Здравствуйте",
  greeting2: "добро пожаловать в ваш личный кабинет!",
  dataTitle: "Ваши данные",
  dataPhone: "Номер",
  dataEmail: "Почта",
  editAlt: "Иконка кнопки редактирования.",
  activeOrderTitle: "Активный заказ",
  activeOrderDeadline: "Заберите заказ до 22:00",
  historyTitle: "История покупок",
  btnText: "Все покупки",
  loyaltyCardTitle1: "Ваша карта лояльности",
  loyaltyCardTitle2: "Ваш баланс баллов",
  logoAlt: "Логотип.",
  pointsText:
    "При любой покупке вам начисляется 3% бонусных баллов Вы можете списать 100% баллов при покупке любого товара",
  addingCard:
    "Хотите копить баллы и экономить свои деньги? Заведите карту лояльности нашего магазина!",
  addingCardText: "Завести карту",
};

export const textShopsData = {
  krasnoarmeiskaya: {
    address: "г. Раменское, ул.Красноармейская, д.4а",
    weekdays: "11:00 – 23:00",
    weekend: "10:00 – 23:00",
    phone: "8 (903) 663-98-87",
  },
  kosmonavtov: {
    address: "г. Раменское, ул.Космонавтов, д.20а",
    weekdays: "10:00 – 23:00",
    weekend: "10:00 – 23:00",
    phone: "8 (926) 040-89-07",
  },
  krymskaya: {
    address: "г. Раменское, ул.Крымская, д.3, пом. 2",
    weekdays: "11:00 – 23:00",
    weekend: "10:00 – 23:00",
    phone: "8 (926) 958-56-93",
  },
  dugina: {
    address: "г. Жуковский, ул.Дугина, д.28/12",
    weekdays: "11:00 – 23:00",
    weekend: "10:00 – 23:00",
    phone: "8 (926) 958-54-98",
  },
  solnechnaya: {
    address: "г. Жуковский, ул. Солнечная, д.12",
    weekdays: "11:00 – 23:00",
    weekend: "10:00 – 23:00",
    phone: "8 (925) 541-31-07",
  },
  chkalova: {
    address: "г. Жуковский, ул. Чкалова, д. 6",
    weekdays: "10:00 – 23:00",
    weekend: "10:00 – 23:00",
    phone: "8 (926) 306-78-58",
  },
  bazhenova: {
    address: "г. Жуковский, ул. Баженова, дом 4/1",
    weekdays: "11:00 – 23:00",
    weekend: "10:00 – 23:00",
    phone: "8 (925) 457-87-20",
  },
  barykina: {
    address: "г. Люберцы, ул. Барыкина дом 1/2",
    weekdays: "11:00 – 23:00",
    weekend: "10:00 – 23:00",
    phone: "8 (925)442-99-72",
  },
  pochtovoe: {
    address: "г. Люберцы, ул. 3-е почтовое отделение, дом 49, корп. 2",
    weekdays: "11:00 – 23:00",
    weekend: "11:00 – 23:00",
    phone: "8 (916) 282-15-68",
  },
  sosnovaya: {
    address: "г. Котельники, ул. Сосновая, дом 1, корп. 5",
    weekdays: "11:00 – 23:00",
    weekend: "11:00 – 23:00",
    phone: "8 (915) 148-86-85",
  },
};
